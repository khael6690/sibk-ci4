    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="container">
                    <form action="<?= base_url('pelanggaran') ?>" method="put" id="form-update" class="form-horizontal">
                        <?= csrf_field() ?>
                        <input type="hidden" value="<?= $data_pelanggaran['pelsiswa_id']; ?>" name="pelsiswa_id" id="pelsiswa_id">
                        <div class="form-group row">
                            <label for="nis" class="col-sm-4 col-form-label">Pilih Siswa</label>
                            <div class="col-sm-8">
                                <select class="form-control select2" id="nis" name="nis">
                                    <?php foreach ($data_siswa as $siswa) { ?>
                                        <option value="<?= $siswa['nis']; ?>" <?= $data_pelanggaran['nis'] == $siswa['nis'] ? 'selected' : ''; ?>><?= $siswa['nama'] . " | " . $siswa['kelas'] . " - " . $siswa['singkatan']; ?></option>
                                    <?php } ?>
                                </select>
                            ...
                            <div class="col-sm-8">
                                <select class="form-control select2" id="kategori_id" name="kategori_id">
                                    <?php foreach ($data_kategori as $kategori) { ?>
                                        <option value="<?= $kategori['kategori_id']; ?>" <?= $data_pelanggaran['kategori_id'] == $kategori['kategori_id'] ? 'selected' : ''; ?>><?= $kategori['nama']; ?></option>
                                    <?php } ?>
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control select2" id="pelanggaran_id" name="pelanggaran_id">
                                    <option value="<?= $data_pelanggaran['pelanggaran_id']; ?>" selected><?= $data_pelanggaran['nama_pelanggaran']; ?></option>
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                                <textarea class="form-control" rows="3" id="keterangan" name="keterangan" placeholder="keterangan ..." style="resize: none;"><?= old('keterangan', $data_pelanggaran['keterangan']) ?></textarea>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="tindakan" class="col-sm-4 col-form-label">Tindakan</label>
                            <div class="col-sm-8">
                                <textarea class="form-control" rows="3" id="tindakan" name="tindakan" placeholder="tindakan ..." style="resize: none;"><?= old('tindakan', $data_pelanggaran['tindakan']) ?></textarea>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.error) {
                    ...
                    response.error && response.error.tindakan ?
                        (form.find('#tindakan').addClass('is-invalid'), form.find('.errortindakan').html(response.error.tindakan)) :
                        (form.find('#tindakan').removeClass('is-invalid'), form.find('.errortindakan').html(''));
                } else {
