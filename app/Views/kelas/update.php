<div class="modal fade" id="modal-update-kelas">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="container">
                    <form action="<?= base_url('kelas') ?>" method="PUT" class="form-horizontal form-update-kelas">
                        <?= csrf_field() ?>
                        <input type="hidden" id="kelas_id" name="kelas_id" value="<?= old('kelas_id', $data_kelas['kelas_id']) ?>">
                        <div class="form-group row">
                            <label for="nama_kelas" class="col-sm-4 col-form-label">Nama Kelas</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" id="nama_kelas" name="nama_kelas" value="<?= old('nama_kelas', $data_kelas['nama_kelas']) ?>" placeholder="XII">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control select2" id="guru_id" name="guru_id">
                                    <?php foreach ($data_guru as $guru) { ?>
                                        <option value="<?= $guru['guru_id']; ?>" <?= $data_kelas['guru_id'] === $guru['guru_id'] ? 'selected' : ''; ?>><?= $guru['nama']; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-8">
                                <select class="form-control select2" id="jurusan_id" name="jurusan_id">
                                    <?php foreach ($data_jurusan as $jurusan) { ?>
                                        <option value="<?= $jurusan['jurusan_id']; ?>" <?= $data_kelas['jurusan_id'] === $jurusan['jurusan_id'] ? 'selected' : ''; ?>><?= $jurusan['singkatan']; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
...
<script>
    ...
    $('.form-update-kelas').submit(function(e) {
        e.preventDefault();
        var form = $(this);
        $.ajax({
